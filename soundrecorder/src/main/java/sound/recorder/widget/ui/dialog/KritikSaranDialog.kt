...
class KritikSaranDialog {
    ...
    private fun showSongRequestDialog(context: Context) {
        ...
        root.addView(TextView(context).apply {
            text = context.getString(R.string.help_suggest)
        ...
        root.addView(TextView(context).apply {
            text = context.getString(R.string.help_us_to_improve)
        ...
        val etDescription = EditText(context).apply {
            hint = context.getString(R.string.input_your_suggest)
            setHintTextColor(Color.parseColor("#666666"))
            setTextColor(Color.WHITE)
            textSize = 14f
            gravity = Gravity.TOP
            ...
            val p = 28
            val tinggi = context.sdp(SdpR.dimen._74sdp)
            setPadding(p, p, p, p)
            val lp = LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT,
                tinggi
        ...
        btnRow.addView(TextView(context).apply {
            ...
            setOnClickListener {
                ...
                if (title.isEmpty()) {
                    ...
                    Toast.makeText(context,context.getString(R.string.name_cannot_empty),
                        Toast.LENGTH_SHORT).show()
                    return@setOnClickListener
                }
                if (desc.isEmpty()) {
                    //etTitle.error = context.getString(R.string.song_title_cannot_empty)
                    Toast.makeText(context,context.getString(R.string.description_cannot_empty),
                        Toast.LENGTH_SHORT).show()
                    return@setOnClickListener
                }
                d.dismiss()
        ...
        FirebaseMessaging.getInstance().token.addOnCompleteListener { task ->
            ...
            FirebaseFirestore.getInstance()
                ...
                .addOnSuccessListener {
                    Toast.makeText(context, context.getString(R.string.thankyou)+" " + name +" "+context.getString(R.string.suggest_success_send) , Toast.LENGTH_LONG).show()
                }
                .addOnFailureListener {
                    Toast.makeText(context, context.getString(R.string.suggest_failed_send), Toast.LENGTH_SHORT).show()
                }
        }
    }
